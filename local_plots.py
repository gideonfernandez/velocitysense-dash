...
def main():
    ...
    comp_df = pd.read_csv("data/bankpoc.csv", index_col=0)
    ...
    # Complaints by date
    fig = px.histogram(
        comp_df, x="Date", template="plotly_white", title="Complaint counts by date"
    )
    ...
    # Pre-processing data
    top_comps = (
        comp_df.groupby("1. Name of Organization")["Date"]
        .count()
        .sort_values(ascending=False)[:10]
        .index
    )
    top_comps_df = comp_df[comp_df["1. Name of Organization"].isin(top_comps)]

    # Top companies by complaints
    fig = px.histogram(
        top_comps_df,
        x="1. Name of Organization",
        template="plotly_white",
    ...
    fig = px.treemap(
        ...
        path=["1. Name of Organization", "credit_report"],
        color="Words",
    ...
    fig = px.bar(
        comp_grp_df,
        x="portion",
        y="1. Name of Organization",
        template="plotly_white",
        orientation="h",
        labels={"portion": "% of Complaints", "bigram": "Bigram", "1. Name of Organization": "1. Name of Organization"},
        color="bigram",
